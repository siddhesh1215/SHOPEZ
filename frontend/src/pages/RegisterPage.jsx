    ...
    return (
        <div className="auth-page">
            <div className="auth-card card animate-in">
                ...
                <p className="auth-sub">Join ShopEZ and start shopping or selling</p>

                {/* Role Toggle */}
                <div className="role-toggle">
                    <button className={`role-btn ${form.role === 'customer' ? 'active' : ''}`} type="button" onClick={() => setForm({ ...form, role: 'customer' })}>🛍️ Customer</button>
                    <button className={`role-btn ${form.role === 'seller' ? 'active' : ''}`} type="button" onClick={() => setForm({ ...form, role: 'seller' })}>🏪 Seller</button>
                </div>
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="form-group">
                        ...
                        <input type="password" className="form-control" placeholder="Minimum 6 characters" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} required />
                    </div>

                    {error && <div className="alert alert-error">{error}</div>}

                    <button type="submit" className="btn btn-primary btn-full btn-lg" disabled={loading}>
                        {loading ? 'Creating Account...' : `Create ${form.role === 'seller' ? 'Seller' : 'Customer'} Account`}
                    </button>
                </form>
